...
the `etude` dogfood repo, use the four-reviewer gate for every phase.
...
5. **Build and globally install the project, then manually exercise the built
   artifact.** If the bead adds or changes a runnable binary/CLI, do not stop at
   `go test`: build and install it globally (for `etude`: `make build` then
   `go install ./cmd/etude`, or the project's equivalent), and run the actual
   installed binary through its real behavior — golden paths and edge cases — in
   a realistic throwaway environment. Automated tests pass with mocked/in-process
   wiring; only running the installed binary proves the shipped command works.
   Record the exact commands run and observed output in the Manual tests section.
6. Assess coverage, correctness, regressions, and code quality.
7. Produce one Verify artifact with status `pass`, `fail`, or `blocked`.
...
8. If the gate passes with a close recommendation, the parent workflow wraps up
   AUTOMATICALLY — do NOT ask the user how to wrap up or whether to commit/push.
   The standing wrap-up is:
   a. Commit the bead's files as one commit (the bead = one commit).
   b. Remove `phase:review`, add `phase:complete`, close the bead with a one-line
      reason that names what landed and the commit SHA.
   c. If Docs flagged any planning notes as stale-on-close, reconcile them and
      commit that as a SEPARATE second commit (keeps the bead commit clean).
   d. `git push` and push bead storage (`bd dolt push`).
   This is the user's standing instruction: always commit + push the completed,
   gate-passed bead without asking. (Overrides the generic "ask before commit"
   default for this gated-bead flow only.)
